...
<!-- 					<div class="top_title">공지사항</div> -->
				...
				<a href="/spgame/user/app_qna.do" class="btn c-button"><spring:message code="top.inquiry" /></a>
					<div class="c_content">
						<c:forEach var="item" items="${list}">
							<div class="list_warp3" onclick="location.href='/spgame/user/app_qna_detail.do?idx=${item.idx}'" style="cursor:pointer;">
								<div class="c_list_title">${item.text}</div>
								<div class="c_date"><fmt:formatDate value="${item.pdate}" pattern="yyyy-MM-dd"/></div>
							</div>
						</c:forEach>
					</div>
					<form name="listForm" action="/spgame/user/app_qnaList.do">
